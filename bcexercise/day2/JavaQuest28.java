...
for(int i=0;i<patterns.length;i++){
  StringBuilder words = new StringBuilder(patterns[i]);
  for(int k=0;k<patterns[i].length();k++){
    words.insert(0,words.charAt(words.length()-1)).delete(words.length()-1,words.length());
    if(word.contains(words.toString())) count++;
  }
}
return count;
}
